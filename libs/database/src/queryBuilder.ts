import { QueryBuilder, Model, Page, OrderByDirection, PrimitiveValue, Expression } from 'objection';
...
export class CustomQueryBuilder<M extends Model, R = M[]> extends QueryBuilder<M, R> {
  ...
  async chunk(cb: GenericFunction, size: number): Promise<void> {
    ...
    while (!!!offset || hasMore) {
      ...
      const records = (await query.offset(offset).limit(size)) as unknown as M[];
